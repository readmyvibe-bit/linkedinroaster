...
function ResumeFormContent() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (orderPlan !== 'pro' && (selectedTemplate as any)?.proOnly) {
      setSubmitError(`"${selectedTemplate?.name}" is a Pro template. Upgrade to Pro for ₹500 to unlock all 23 templates + 3 resumes. Your Standard payment of ₹299 will be adjusted.`);
      return;
    }
    setSubmitError('');
    setSubmitting(true);
